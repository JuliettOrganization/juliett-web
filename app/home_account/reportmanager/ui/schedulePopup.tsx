    ...
    if (reportid) {
      fetch(`/api/home_account/reportmanager/getReportScheduleDetails`, {
        ...
        .then((reportData) => {
          ...
          setFirstDayOfYear(
            reportData.schedulefirstdayofyear
              ? new Date(reportData.schedulefirstdayofyear)
              : new Date(),
          );
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch(
        "/api/home_account/reportmanager/updateScheduler",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 w-96">
        ...
        <div className="mb-4">
          <label className="block mb-2">
            Year Start (only day and month will be used)
          </label>
          <input
            id="firstDayOfYear"
            name="firstDayOfYear"
