...
import ReportStatus from "@/app/home_account/reportmanager/ui/status";
import ReportTags from "@/app/home_account/reportmanager/ui/tags";
import DropDownMenu from "@/app/home_account/reportmanager/ui/DropDownMenu";
...
import { useReportActions } from "@/app/home_account/reportmanager/ui/buttons";
...
const ReportsTableClient: React.FC<ReportsTableClientProps> = ({
  query,
  currentPage,
}) => {
    ...
    if (accountid) {
      ...
      try {
        ...
        const response = await fetch(
          `/api/home_account/reportmanager/fetchFilteredReports?accountid=${accountid}&query=${encodeURIComponent(query)}&page=${currentPage}&sort=${sortParam}`,
        );
        if (!response.ok) {
        ...
        <div className="flex flex-col rounded-lg w-full bg-gray-50 p-2 md:pt-0">
          <div className="md:hidden">
            {sortedReports.map((report) => (
              ...
              >
                <div>
                  <p className="text-sm text-gray-500">{report.reportname}</p>
                </div>
                <div>
                ...
                <div className="flex justify-end gap-2 relative">
                  <DropDownMenu
                    ...
                    handleDelete={(reportId) =>
                      handleDelete(reportId, setPopupMessage, setErrors)
                    }
                    handleClone={(reportId) =>
                      handleClone(reportId, setPopupMessage, setErrors)
                    }
                  />
                </div>
              </div>
            ...
            <thead className="text-left min-w-full text-sm font-normal overflow-x-auto">
              <tr>
                <th
                  ...
                  className="px-4 py-5 text-left text-sm font-medium cursor-pointer tracking-wider"
                >
                  Report Name {getSortIcon("reportname")}
                </th>
                <th
                  onClick={() => handleSort("status")}
                  className="px-4 py-5 text-sm text-left font-medium cursor-pointer tracking-wider"
                >
                ...
                <th
                  onClick={() => handleSort("date_concept")}
                  className="px-4 py-5 text-left text-sm font-medium cursor-pointer tracking-wider"
                >
                ...
                <th
                  onClick={() => handleSort("isscheduleractive")}
                  className="px-4 py-5 text-left text-sm font-medium cursor-pointer tracking-wider"
                >
