...
const initialSelectedClass: SelectedClassType = rbds.reduce<SelectedClassType>(
  (acc, rbd) => {
    acc[rbd] = "Economy";
    return acc;
  },
...
const ClassMappingTable: React.FC = () => {
  const [selectedClass, setSelectedClass] =
    useState<SelectedClassType>(initialSelectedClass);
  ...
  const handleSave = () => {
    ...
    console.log(
      "%cSelections have been saved!",
      "color: white; background-color: purple; font-size: 16px; padding: 4px; border-radius: 4px;",
    );
          ...
          <table className="min-w-full bg-white rounded-lg relative">
            <thead className="bg-gray-500 rounded-lg text-white sticky top-0 z-10">
              <tr>
                ...
                <th className="py-2 px-4 border-b w-[22.5%]">
                  Premium Economy
                </th>
                ...
                <tr key={rbd}>
                  ...
                  {["Economy", "Premium Economy", "Business", "First"].map(
                    (classType) => (
                      <td
                        key={classType}
                        className="py-2 px-4 border-b text-center w-[22.5%]"
                      >
                        <input
                          type="radio"
                          name={rbd}
