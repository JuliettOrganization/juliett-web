...
const AnalyticsPage = () => {
  ...
  const fetchChartData = async () => {
    ...
    setLoading(true); // Show loading spinner
    try {
      const response = await fetch(`/api/home_account/analytics/fetchChartData?accountid=${accountid}&reportid=${selectedReportid}&startDate=${startDate.toISOString()}&endDate=${endDate.toISOString()}`);
      if (!response.ok) {
    ...
    <div className="bg-gray-300 min-h-screen p-2">
            ...
            <div className="flex flex-row space-x-2 items-center">
              <label htmlFor="date_range" className="block text-sm font-medium">
                ...
                <DatePicker
                  ...
                  popperContainer={({ children }) => <div className="fixed z-50">{children}</div>}
                />
                </div>
            </div>
        ...
        <div className="col-span-4 bg-white p-4 rounded-lg mb-4">
          <Tabs defaultValue="Day" className="w-full">
            ...
            <TabsContent value="Day">
                {activeTab === 0 && chartData && chartData[`daily${selectedKPI.trim().replace(/\s+/g, '')}`] && selectedKPI && (
                <div className="justify-center">
                  <Line
                    data={chartData[`daily${selectedKPI.trim().replace(/\s+/g, '')}`]}
                    options={{
                      elements: {
                        line: {
           ...
           <TabsContent value="Week">
             ...
             <TabsContent value="Month">
              {activeTab === 2  && chartData && chartData[`monthly${selectedKPI.trim().replace(/\s+/g, '')}`] && selectedKPI && (
                <div className="justify-center">
                 <Bar
                                      data={chartData[`monthly${selectedKPI.trim().replace(/\s+/g, '')}`]}
                                      options={{
                                      scales: {
                                        y: {
                                          title: {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white p-4 rounded-lg">
          <h2 className="text-lg font-bold mb-4">Airline Wise {`${selectedKPI} ${
                                              selectedKPI === 'Gross Amount' || selectedKPI === 'Average Fare'
                                                ? `(${reportInfo ? reportInfo[0].currency : 'USD'})`
                                                : ''
                                            }`}</h2>
            {chartData && chartData[`AirlineData${selectedKPI.trim().replace(/\s+/g, '')}`] && selectedKPI && (
             <Doughnut 
                  data={{
         ...
         <div className="bg-white p-4 rounded-lg">
                 <h2 className="text-lg font-bold mb-4">Cabin Wise {`${selectedKPI} ${
                                              selectedKPI === 'Gross Amount' || selectedKPI === 'Average Fare'
                                                ? `(${reportInfo ? reportInfo[0].currency : 'USD'})`
                                                : ''
                                            }`}</h2>
                 {chartData && chartData[`ClassData${selectedKPI.trim().replace(/\s+/g, '')}`] && selectedKPI &&  <Doughnut data={chartData[`ClassData${selectedKPI.trim().replace(/\s+/g, '')}`]} />}
               </div>
               <div className="bg-white p-4 rounded-lg">
                 <h2 className="text-lg font-bold mb-4">Agency Wise {`${selectedKPI} ${
                                              selectedKPI === 'Gross Amount' || selectedKPI === 'Average Fare'
                                                ? `(${reportInfo ? reportInfo[0].currency : 'USD'})`
                                                : ''
                                            }`}</h2>
                 {chartData && chartData[`AgencyData${selectedKPI.trim().replace(/\s+/g, '')}`] && selectedKPI && (
                 <Bar
                   data={chartData[`AgencyData${selectedKPI.trim().replace(/\s+/g, '')}`]}
                   options={{
                     indexAxis: 'y',
                     scales: {
                       x: {
