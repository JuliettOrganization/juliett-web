...
const LoadingPage = () => {
    ...
    const fetchCountriesData = async () => {
      try {
        const response = await fetch(
          `/api/home_account/loadingStatus?accountid=${accountid}`,
        );
        if (response.ok) {
  ...
  const filteredCountriesData = countriesData
    ...
    .map((countryData) => {
      const filteredYears = Object.entries(countryData.years).reduce(
        (acc, [year, yearData]) => {
          const filteredMonths = Object.entries(yearData.months).reduce(
    ...
    .filter((countryData) =>
      Object.keys(countryData.years).some(
        (year) => Object.keys(countryData.years[year].months).length > 0,
      ),
    );
  ...
  const toggleAllYears = () => {
    ...
    const newExpandedYears = filteredCountriesData.reduce(
      (acc, countryData) => {
    ...
    <div className="flex flex-col m-2 justify-start">
      <div className="flex flex-col mb-8">
        ...
        <p className="text-lg text-gray-500">
          Check the loading status for each country available in your account.
        </p>
      </div>
              ...
              <div key={year} className="mb-4">
                ...
                {expandedYears[year] && (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {Object.entries(yearData.months).map(
                      ([month, loadedDays]) => (
                        <div
                          key={month}
