...
const GroupingValuesFilter: React.FC<GroupingValuesFilterProps> = ({
  ...
  return (
    <div className="mb-4 mr-2 relative w-128">
      <label
        htmlFor="AirlineGroupingsIssuing"
      ...
      <div className="flex flex-row w-full items-center peer cursor-pointer bg-white rounded-md border border-gray-200 pt-2 pb-1 pl-2 mr-2 ml-auto text-sm text-left outline-2 placeholder:text-gray-500">
        <button
          type="button"
          onClick={toggleDropdownIssuing}
          className="flex flex-row w-full text-ellipsis overflow-hidden whitespace-nowrap space-x-3"
        >
          <span className="truncate">
            {selectedGroupingValuesIssuing.length > 0
              ? selectedGroupingValuesIssuing.join(", ")
              : "Select Grouping"}
          </span>
...
const CheckBoxIssuing: React.FC<CheckBoxIssuingProps> = ({
  ...
  useEffect(() => {
    if (
      selectedGroupingIssuing !== "" &&
      selectedGroupingIssuing !== prevSelectedGroupingIssuing.current
    ) {
      ...
      fetch(
        `/api/home_account/reportdesign/airlinefilter/airlineGroupingValues?selectedGrouping=${selectedGroupingIssuing}`,
      )
