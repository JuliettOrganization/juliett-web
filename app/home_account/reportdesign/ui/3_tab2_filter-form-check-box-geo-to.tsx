  ...
  return (
    <div className="mb-4 mr-2 relative w-128">
      ...
      <div className="flex flex-row w-full items-center peer cursor-pointer bg-white rounded-md border border-gray-200 pt-2 pb-1 pl-2 mr-2 ml-auto text-sm text-left outline-2 placeholder:text-gray-500">
        <button
          type="button"
          onClick={toggleDropdownGeoTo}
          className="flex flex-row w-full text-ellipsis overflow-hidden whitespace-nowrap space-x-3"
        >
          <span className="truncate">
            {selectedGroupingValuesGeoTo.length > 0
              ? selectedGroupingValuesGeoTo.join(", ")
              : "Select Grouping"}
          </span>
...
const CheckBoxGeoTo: React.FC<CheckBoxGeoToProps> = ({
  ...
  useEffect(() => {
    if (
      selectedGroupingGeoTo !== "" &&
      selectedGroupingGeoTo !== prevSelectedGroupingGeoTo.current
    ) {
      ...
      fetch(
        `/api/home_account/reportdesign/geofilter/geoGroupingValues?selectedGrouping=${selectedGroupingGeoTo}`,
      )
