...
const GroupingValuesFilter: React.FC<GroupingValuesFilterProps> = ({
  ...
  return (
    <div className="mb-4 mr-2 relative w-128">
      <label
        htmlFor="GeoFromGroupingsGeoFrom"
      ...
      <div className="flex flex-row w-full items-center peer cursor-pointer bg-white rounded-md border border-gray-200 pt-2 pb-1 pl-2 mr-2 ml-auto text-sm text-left outline-2 placeholder:text-gray-500">
        <button
          type="button"
          onClick={toggleDropdownGeoFrom}
          className="flex flex-row w-full text-ellipsis overflow-hidden whitespace-nowrap space-x-3"
        >
          <span className="truncate">
            {selectedGroupingValuesGeoFrom.length > 0
              ? selectedGroupingValuesGeoFrom.join(", ")
              : "Select Grouping"}
          </span>
...
const CheckBoxGeoFrom: React.FC<CheckBoxGeoFromProps> = ({
  ...
  useEffect(() => {
    if (
      selectedGroupingGeoFrom !== "" &&
      selectedGroupingGeoFrom !== prevSelectedGroupingGeoFrom.current
    ) {
      ...
      fetch(
        `/api/home_account/reportdesign/geofilter/geoGroupingValues?selectedGrouping=${selectedGroupingGeoFrom}`,
      )
