...
import FilterForm from "./3_tab2_filter-form";
...
const CreateFormLayout: React.FC<CreateFormLayoutProps> = ({
  ...
  const availableFields = [
    ...
    "date (travel)",
    "deal code (GNR5)",
    "destination (coupon level)",
    "destination airport code (ticket level)",
    "destination city code (coupon level)",
    "destination city code (ticket level)",
    "destination country code (ticket level)",
    "destination country code (coupon level)",
    ...
    "origin (coupon level)",
    "origin airport code (ticket level)",
    "origin city code (coupon level)",
    "origin city code (ticket level)",
    "origin country code (ticket level)",
    "origin country code (coupon level)",
    ...
    "stopover",
    "taxes",
    "ticket number (original reference)",
    "ticket number (primary)",
    "ticket number (refunded)",
    "ticket number (TDNR)",
    "tour code",
    "transaction code",
    "yq amount (local currency)",
    "yr amount (local currency)",
  ];
  const [searchQuery, setSearchQuery] = useState<string>("");
  const [activeTab, setActiveTab] = useState<number>(0);
  const [isDropdownOpenAgency, setIsDropdownOpenAgency] =
    useState<boolean>(false);
  const [isDropdownOpenIssuing, setIsDropdownOpenIssuing] =
    useState<boolean>(false);
  const [isDropdownOpenMarketing, setIsDropdownOpenMarketing] =
    useState<boolean>(false);
  const [isDropdownOpenOperating, setIsDropdownOpenOperating] =
    useState<boolean>(false);
  const [isDropdownOpenGeoFrom, setIsDropdownOpenGeoFrom] =
    useState<boolean>(false);
  const [isDropdownOpenGeoTo, setIsDropdownOpenGeoTo] =
    useState<boolean>(false);
  const toggleDropdownAgency = () => {
    setIsDropdownOpenAgency(!isDropdownOpenAgency);
  };
  const toggleDropdownIssuing = () => {
    setIsDropdownOpenIssuing(!isDropdownOpenIssuing);
  };
  const toggleDropdownMarketing = () => {
    setIsDropdownOpenMarketing(!isDropdownOpenMarketing);
  };
  const toggleDropdownOperating = () => {
    setIsDropdownOpenOperating(!isDropdownOpenOperating);
  };
  const toggleDropdownGeoFrom = () => {
    setIsDropdownOpenGeoFrom(!isDropdownOpenGeoFrom);
  };
  const toggleDropdownGeoTo = () => {
    setIsDropdownOpenGeoTo(!isDropdownOpenGeoTo);
  };
  ...
  const [benchmarkPeriod, setBenchmarkPeriod] = useState<string>(
    initialBenchmarkPeriod,
  );
  const [benchmarkDateFrom, setBenchmarkDateFrom] = useState<string>(
    initialBenchmarkDateFrom,
  );
  const [benchmarkDateTo, setBenchmarkDateTo] = useState<string>(
    initialBenchmarkDateTo,
  );
  ...
  const [isCustomSqlActive, setIsCustomSqlActive] = useState<boolean>(
    initialIsCustomSqlActive,
  );
  const [selectedGroupingAgency, setSelectedGroupingAgency] = useState<string>(
    initialSelectedGroupingAgency,
  );
  const [selectedGroupingGeoFrom, setSelectedGroupingGeoFrom] =
    useState<string>(initialSelectedGroupingGeoFrom);
  const [selectedGroupingGeoTo, setSelectedGroupingGeoTo] = useState<string>(
    initialSelectedGroupingGeoTo,
  );
  const [selectedGroupingIssuing, setSelectedGroupingIssuing] =
    useState<string>(initialSelectedGroupingIssuing);
  const [selectedGroupingMarketing, setSelectedGroupingMarketing] =
    useState<string>(initialSelectedGroupingMarketing);
  const [selectedGroupingOperating, setSelectedGroupingOperating] =
    useState<string>(initialSelectedGroupingOperating);
  const [selectedGroupingValuesAgency, setSelectedGroupingValuesAgency] =
    useState<string[]>(initialSelectedGroupingValuesAgency);
  const [selectedGroupingValuesGeoFrom, setSelectedGroupingValuesGeoFrom] =
    useState<string[]>(initialSelectedGroupingValuesGeoFrom);
  const [selectedGroupingValuesGeoTo, setSelectedGroupingValuesGeoTo] =
    useState<string[]>(initialSelectedGroupingValuesGeoTo);
  const [selectedGroupingValuesIssuing, setSelectedGroupingValuesIssuing] =
    useState<string[]>(initialSelectedGroupingValuesIssuing);
  const [selectedGroupingValuesMarketing, setSelectedGroupingValuesMarketing] =
    useState<string[]>(initialSelectedGroupingValuesMarketing);
  const [selectedGroupingValuesOperating, setSelectedGroupingValuesOperating] =
    useState<string[]>(initialSelectedGroupingValuesOperating);
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch(
        "/api/home_account/reportdesign/saveReport",
        {
          method: "POST",
          headers: {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      const excelResponse = await fetch(
        "/api/home_account/reportdesign/generateExcel",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    <main className="z-20">
      ...
      <div className="flex flex-col lg:mr-48 mr-44 ml-0 mt-0 h-full border-none marker:overflow-y-auto">
        ...
        <Tabs defaultValue="main-options" className="w-full">
          <TabsList>
            <TabsTrigger value="main-options" onClick={() => setActiveTab(0)}>
              Main Options
            </TabsTrigger>
            <TabsTrigger
            ...
            <TabsTrigger value="filters" onClick={() => setActiveTab(2)}>
              Filters
            </TabsTrigger>
            <TabsTrigger value="custom-filters" onClick={() => setActiveTab(3)}>
              Custom Filters
            </TabsTrigger>
          </TabsList>
            ...
            {activeTab === 1 && (
              <div className="p-4 bg-white rounded-lg shadow-md">
                <div className="bg-white rounded-lg">
                  <div className="flex flex-row rounded-md p-4 justify-start text-xl font-bold space-x-6">
                    Field Selection
                  </div>
              ...
              <div className="p-4 bg-white rounded-lg shadow-md">
                <div className="flex flex-col w-full p-6 items-center bg-white rounded space-y-4">
                  <ToggleSwitchCustomSql
                    isCustomSqlActive={isCustomSqlActive}
                    setIsCustomSqlActive={setIsCustomSqlActive}
                  />
                </div>
