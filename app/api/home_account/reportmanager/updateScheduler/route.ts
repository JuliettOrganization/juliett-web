...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ message: 'Report scheduled successfully', updateQuery,updateValues  }, { status: 200 });
  } catch (error) {
    console.error('Error scheduling report:', error);
    return NextResponse.json({ error: 'Failed to scheduled report'}, { status: 500 });
  }
}