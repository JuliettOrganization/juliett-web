...
import UserStatus from "@/app/home_user/admin/users/ui/1_userstatus_style";
import EmailConfirmation from "@/app/home_user/admin/users/ui/1_useremailconfirmation_style";
import AccountNames from "@/app/home_user/admin/users/ui/1_accountnames_style";
          ...
          <div className="md:hidden">
            {users?.map((user) => (
              <div key={user.id} className="mb-2 w-full rounded-md shadow p-4">
                <div>
                  <p className="text-sm text-purple-500">{user.name}</p>
                </div>
                <div>
                  <p className="text-sm text-purple-500">{user.email}</p>
                </div>
                <div>
                  <p className="text-sm text-purple-500">{user.role}</p>
                </div>
                <div>
                  <EmailConfirmation status={user.confirmationstatus} />
                </div>
                <div>
                  <UserStatus status={user.accountstatus} />
                </div>
                <div>
                  <AccountNames accountnames={user.accountnames} />
                </div>
                <div className="flex justify-end gap-2 relative">
                  ...
                  {activeMenu === user.id && (
                    <div
                      ref={menuRef}
                      className="absolute right-0 mr-2 bg-white shadow-lg rounded w-48"
                    >
                      <ul>
                  ...
                  <td className="whitespace-nowrap py-3 pl-6 pr-3 relative">
                    ...
                    {activeMenu === user.id && (
                      ...
                      >
                        <ul>
                          <li className="px-4 py-2 hover:bg-gray-100 cursor-pointer">
                            Edit User (AWS Cognito)
                          </li>
                          <li className="px-4 py-2 hover:bg-gray-100 cursor-pointer">
                            Delete User (AWS Cognito)
                          </li>
                        </ul>
                      </div>
                    )}
