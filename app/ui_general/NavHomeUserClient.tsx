...
const NavHomeUserClient = () => {
  ...
  useEffect(() => {
    // Dynamically add preload link with correct 'as' attribute
    const preloadLink = document.createElement('link');
    preloadLink.href = profile.src;
    preloadLink.rel = 'preload';
    preloadLink.as = 'image'; // Specify the correct 'as' value
  ...
  const handleSignOut = async () => {
    try {
      ...
      if (response.ok) {
        ...
        window.location.href = "/"; // Redirect to the home page
        console.error(
          "Failed to sign out CLIENT SIDE",
          response.status,
          errorText,
        );
      }
      ...
      <div className="flex justify-end relative">
        ...
        >
          ...
          <div className="flex flex-col text-left">
            ...
            <span className="hidden md:block text-xs text-left text-gray-400">
              {email}
            </span>
          </div>
